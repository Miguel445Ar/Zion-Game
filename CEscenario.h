...
public:
	...
	bool colision(int x, int y, int ancho, int alto) {
		return (area_dibujo.IntersectsWith(RECTANGULO(x, y, ancho, alto)));
	}
};
...
public:
	...
	void get_pos_muro(int& x, int& y) {
		int x1, y1;
		while (true) {
			x1 = aleatorio(1, M_size - 2);
			y1 = aleatorio(1, M_size - 2);
			if (M[y1][x1] == 1) {
				x = x1;
				y = y1;
				break;
	...
	void get_pos_meta(int& x, int& y) {
		int y1;
		while (true) {
			y1 = aleatorio(1, M_size - 1);
			if (M[y1][M_size - 2] == 0) {
				x = M_size - 2;
				y = y1;
				break;
...
public:
	...
	void mostrar_muros(Graphics^ g, Bitmap^ img) {
		...
		TextureBrush^ t_brush = gcnew TextureBrush(img);
		g->FillRectangle(t_brush, 0, 0, 32 * M_size, 30 * M_size);
	}
	...
	bool determinar_colision_muro(int x, int y, int ancho, int alto) {
		for (int i = 0; i < muros.Count; ++i) {
			if (muros[i]->colision(x, y, ancho, alto) == true) {
				return true;
			}
		}
		return false;
